...
<body>
  ...
  <form action="wx/personal_dog_info" id="hform" method="post">
  
		<div class="weui-cell">
      <div class="weui-cell__hd"><label for="" class="weui-label">身份证号码：</label></div>
      <div class="weui-cell__bd" >
				<input id="userid" type="text" name="IDNum" value="" pattern="^\d{15}$|^\d{17}([0-9]|X)$" required oninvalid="setCustomValidity('请输入有效的身份证号码');" >
      </div>
		</div>
    <div class="weui-cell">
      <div class="weui-cell__hd"><label for="" class="weui-label">犬证号码：</label></div>
      <div class="weui-cell__bd">
				<input id="dogid" type="text" name="registrationNum" value="" pattern="^\S+$" maxlength="30" required oninvalid="setCustomValidity('请输入犬证号码');">
      </div>
		</div>
    <div class="weui-cell">
      <div class="weui-cell__hd"><label for="" class="weui-label"></label></div>
        <div class="weui-cell__bd">
          <p><span>*</span>&nbsp;请输入身份证号码以及犬证号码</p>
        </div>
    </div>
	...
	<script>
	  ...
	  $(".submit").click(function(){
	      if (!$("#userid").val()||$("#userid").val().search(/^\d{15}$|^\d{17}([0-9]|X)$/)==-1) {
	        ...
	        return false;
	      }else if (!$("#dogid").val()||$("#dogid").val().search(/^\S+$/)==-1) {
	        $.toptip('请输入犬证号码',2000,'warning');
	        return false;
	      }else {
	        $("#hform").submit();
	      }
